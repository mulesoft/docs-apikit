...
=== What is transform to SQL select in OData 4?

The `transform-to-sql-select` is an operation that allows the user to transform an OData query request into a SQL select statement. It has been designed to work with the Mule DB connector to help the user expose databases with OData 4 easily.
...
* Parameter Aliases
* Built-in Filter Operations (has, divby) 
* Built-in Query Functions (all but contains, startswith, endwith)
* $apply
* $compute
* $expand
...
| Page size | A number that defines the amount of pages in server-side-pagination.
