...
Build an OData v4 API with APIkit for OData using CSDL metadata, scaffolding, and the xref:creating-an-odatav4-api-with-apikit.adoc#apikit-for-odata-4-implementation-example[OData MySQL example] to query MySQL over HTTP. The tutorial covers project setup, generated flows, and running the packaged sample.
