...
As OData shares some similarities with Java database connectivity (JDBC), the `transform-to-sql-select` operation takes advantage of such similarities to help the user create SQL queries for those resources. The `transform-to-sql-select` operation returns the query to the payload along with the query parameters as the attributes.
