...
== Scaffold an OData v4 API Using the APIkit for OData

. In Studio, select *File* > *New* > *Mule Project*.
. In the *Project Name* field, type the name of your Mule project.
. Click *Finish*.
. Create a new file with extension _.csdl.xml_ in `+src/main/resources/api+`. For example, `odata-metadata.csdl.xml`:
+
[source,xml]
--
...
<1> A flow with `GET` operation to retrieve the collection of entities.
...
APIkit for OData 4 contains a `Transform to SQL Select` operation to transform the input OData `GET` request to an SQL SELECT query. The operation is useful when combined with the `select` operation of the Database Connector. The output payload is a prepared statement and the attributes is a map of prepared statement parameters. 
