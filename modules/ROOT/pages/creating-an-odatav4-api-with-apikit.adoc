...
​​The `$filter` expression in OData uses comparison, logical, arithmetic, and grouping operators. You can see the list operators with their corresponding meaning at http://docs.oasis-open.org/odata/odata/v4.01/cs01/part1-protocol/odata-v4.01-cs01-part1-protocol.html#sec_BuiltinFilterOperations[the OData4 Specification]. The only unsupported operators are `has` and `divby`.
