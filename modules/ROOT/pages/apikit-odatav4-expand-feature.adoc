...
The `generic expand` feature dispatches additional queries to the application when the original flow completes. Because this is done based on the application `EDM`, you need to make extra annotations to add enough context to the module:
...
If you implement `$expand` manually for some relationships, leave these relationships out of the list on the *Expandable Navigation Properties* option. Manual implementations inside the flow are needed for the navigation properties that don't use the `generic expansion` feature.
