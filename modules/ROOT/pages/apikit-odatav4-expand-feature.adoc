...
You can reduce the amount of incoming network traffic and make APIs easier to consume with the `$expand` system query option. Because API consumers request only inline representations of the related entities of query results, the returned information avoids multiple network roundtrips. This benefits metered and latency-sensitive consumers.
...
If using the `$expand` feature, the request` to retrieve a list of employees with their `Manager` navigation property expanded is `+GET https://example.com/api/Employees?$expand=Manager+.

== The `generic expand` Feature

The `generic expand` feature enables you to reuse existing flow implementations as a starting point for expanding requests. Using this feature, you can focus on the relevant data retrieval operations while the module performs the necessary steps to ensure that the `$expand` fields are complete.
...
Internal requests use it when the value of `Property` is matched against the value of `ReferencedProperty`.
