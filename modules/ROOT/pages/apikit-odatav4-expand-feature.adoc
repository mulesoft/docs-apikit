= OData $expand in APIkit for OData v4
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

The OData `$expand` system query option returns related entities inline so clients avoid extra round trips and smaller payloads on metered or high-latency networks. APIkit for OData v4 adds automatic routing for common `$expand` scenarios from module 1.3.0 onward; you configure listeners and optional generic expansion so existing flows can satisfy expanded queries. See the https://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part2-url-conventions.html[OASIS URL conventions] for full `$expand` syntax.

== Before You Begin

Prerequisites for using `$expand` include:
