...
| Database Schema a| String |  The name of the database schema to which the entity set belongs. If not empty, it will be prefixed to the table name in the WHERE clause. |  | {nbsp}
...
** LIMIT
** FETCH_NEXT_ROWS_ONLY |  Option to limit the number of rows |  | {nbsp}
| Sql Identifiers Delimiter a| Enumeration, one of:

** DOUBLE_QUOTES
** BRACKETS
** BACK_TICKS |  Option to delimit identifiers |  | {nbsp}
