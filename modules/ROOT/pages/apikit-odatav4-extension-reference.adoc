...
| Database Schema | String | The name of the database schema to which the entity set belongs. If not empty, it is prefixed to the table name in the WHERE clause. |  | {nbsp}
...
| Expandable navigation properties | List<String> | List of types that can be generically expanded from this flow. | `[]` | {nbsp}
...
| Supported System Query Options - `$expand` a| Boolean |  Wether to enable support for `$expand` or not. |  `true` | {nbsp}
| Ignore errors on expansion enabled a| Boolean |  Disallows errors from flows called by generic expansion to be propagated to this flow. When `true`, failed expansions render as `null`. |  `false` | {nbsp}
| Expandable navigation properties | List<String> | List of types that can generically expanded from this flow. | `[]` | {nbsp}
