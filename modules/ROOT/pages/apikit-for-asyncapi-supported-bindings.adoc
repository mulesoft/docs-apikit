...
| Anypoint Connector for MQ (Anypoint MQ Connector) | `destination` | `destination` | 

By default, the AnypointMQ Connector uses the channel name as the `destination`. You must specify the `destination` (queue or exchange) name if the channel name differs from the actual `destination` name, or if the channel name isn’t a valid `destination` name.
...
.3+| Solace PubSub+ Connector

| `deliveryMode` | `deliveryMode` | The Solace PubSub+ Connector operates in `direct` or `persistent` mode. The default mode is `persistent`. The `deliveryMode` parameter determines the quality of service for publishing messages. The `Direct` mode offers lower latency but doesn’t guarantee message delivery. The ‘persistent’ mode ensures that messages are reliably delivered, even in the event of a broker failure.

| See https://github.com/asyncapi/bindings/blob/master/solace/README.md[Solace Bindings].

| `destinationType` | `destinationType` | The Solace PubSub+ Connector supports `queue` in APIkit for AsyncAPI. | See https://github.com/asyncapi/bindings/blob/master/solace/README.md[Solace Bindings].

| N/A | `topicSubscriptions` | The Solace PubSub+ Connector specifies the list of topics the queue subscribes to when the `destinationType` is `queue`. If you specify no topic, the `queue` automatically subscribes to the topic the channel name represents. | See https://github.com/asyncapi/bindings/blob/master/solace/README.md[Solace Bindings].
