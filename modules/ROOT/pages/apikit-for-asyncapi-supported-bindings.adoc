...
| destination
| destination
| Optional, defaults to the channel name. The destination (queue or Exchange) name for this channel. SHOULD only be specified if the channel name differs from the actual destination name, such as when the channel name isn't a valid destination name in Anypoint MQ.
...
| N/A
| N/A
| If a Schema Registry is used when performing this operation, tells where the ID of the schema is stored (for example, header or payload).

| schemaIdPayloadEncoding
| N/A
| N/A
| Number of bytes or vendor-specific values when schema ID is encoded in the payload (for example, confluent/ apicurio-legacy / apicurio-new).
...
| destinationType
| destinationType
| Currently only ‘queue’ is supported in asynckit EDA.
