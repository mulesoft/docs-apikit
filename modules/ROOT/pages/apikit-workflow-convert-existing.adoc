...
. Specify a listener path for the APIkit router by entering `/api/*` in `Path` field.
...
+
* In the *Flow* drop-down, select `GET-Users-FLOW`.
+
* Click *OK*.
+
...
With this approach you get the APIkit router configuration, the error handler, and the console flow out of the box. You also get a flow that maps each resource.
...
* Delete the scaffolded flows for each resource and map the original flows as described in xref:apikit-workflow-convert-existing.adoc#step-2-map-api-resources-actions-to-flows[].
* Remove all operations from the scaffolded flows and add a flow reference with all the removed operations instead. By choosing this option you introduce an additional layer of indirection because you reference all flows to the flow that has the operations, and such indirection can be inconvenient in some scenarios.
