...
Convert Mule applications that implement HTTP and database logic into APIkit for REST projects so you expose the same behavior through a RAML or OpenAPI Specification (OAS) contract. Use the xref:apikit-workflow-convert-existing.adoc#manual-approach[manual approach] to wrap APIkit Router around existing flows, or the xref:apikit-workflow-convert-existing.adoc#scaffolding-approach[scaffolding approach] to rebuild flows from the API specification if regenerating structure matches your delivery plan. Balance complexity, risk, and how much of the flow graph you can refactor.

The walkthrough uses a sample Mule application that retrieves users from a database:
