...
Whether you have chosen a backend-first design approach or already have a Mule application containing business logic that you wish to expose as a REST service, it is possible to convert your existing Mule Project into an APIkit project.
...
To accomplish this, you can refer to the options provided in the "Design Your API Specification" section of the document xref:apikit-4-implement-rest-api.adoc[Creating, Testing, and Implementing an API]. To illustrate this example, please create or copy the identical API specification and JSON example used in that workflow to the `/src/main/resources/api` folder.

=== Step 2: Create a Main Flow

Within the APIkit project, construct a main flow that will serve as the entry point.
...
A new XML file with the same name as the API will be created containing the scaffolded flows.

_Note that the main advantage of this approach is that you get the APIkit Router configuration, an error handler and the console flow out of the box, in addition to each flow that map the resources._
...
It is important to consider the size and complexity of the application when deciding on the best approach.
