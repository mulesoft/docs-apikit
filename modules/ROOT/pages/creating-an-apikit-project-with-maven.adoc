...
The xref:apikit-basic-anatomy.adoc[`api.raml`] file in `src/main/api` is initially empty. To populate the file, complete one of the following tasks:
...
You need to generate xref:apikit-basic-anatomy.adoc#backend-flows[backend flows] for each resource-action pairing in the RAML you create.
...
+
image::import-maven.png["Components in the *Mule* folder, in the *Import* dialog."]
+
...
. xref:apikit-using.adoc#backend-first-design-process[Enhance backend flows].
