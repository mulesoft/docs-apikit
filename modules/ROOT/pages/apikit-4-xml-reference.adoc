...
|`apikit:flow-mapping` |Maps a RAML resource and action to a specific Mule flow |No
...
Use the `apikit:flow-mapping` element to route a specific RAML resource and action to an existing Mule flow.

[%header,cols="22a,10a,68a"]
|===
|Attribute Name|Required |Description
|`resource` |Yes |RAML resource path to map, for example `/users`.
...
<apikit:config name="api-config" api="api/api.raml">
  <apikit:flow-mappings>
    <apikit:flow-mapping resource="/users" action="get" flow-ref="getUsersFlow" />
  </apikit:flow-mappings>
</apikit:config>
----
