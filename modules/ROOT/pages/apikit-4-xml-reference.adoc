...
== APIkit for REST System Property

[%header]
|===
|System Property | Description
| `apikit.multipart.size.limit` | Validates the content length in multipart requests by specifying the maximum allowable size of multipart content in bytes within an API so requests exceeding this limit are rejected to maintain the desired content size restrictions.
|===
