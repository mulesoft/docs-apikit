...
| `mule.xml.expandExternalEntities` | false | Allows parser to expand XML external entities. Default is `false` to prevent attacks. | 1.3.7
| `mule.xml.expandInternalEntities` | false | Allows parser to expand XML internal entities. Default is `false` to prevent attacks. | 1.3.7
...
| `raml.json_schema.fail_on_warning` | false | Fails when there is a `warning` while validating a payload when RAML parser is configured. | 1.1.15
...
| `mule.apikit.parser` | AUTO | Overrides parser type. Possible values:
...
* AUTO 
| 1.3.7
| `apikit.multipart.size.limit` a| 268435456 | This property limits the size of the multipart payload to be validated throwing InvalidFormParameterException in the case the incoming multipart payload exceeds the limit to avoid stack overflow errors | 1.8.0
| `fullDomain` | localhost | This property overrides the console testing to url | 1.1.15
|===
