...
|===
|Attribute Name|Required |Description
|`disableValidations` |No | `disableValidations` disables validations of requests.
|`headerStrictValidation` |No | `headerStrictValidation` validates that request headers are defined in RAML file or belong to a well-known set of headers.
|`queryParamsStrictValidation` |No | `queryParamsStrictValidation` validates that request query parameters are defined in RAML file.
|===
