...
The xref:3.x@apikit-basic-anatomy.adoc[`api.raml`] file in `src/main/api` is initially empty. To populate the file, complete one of the following tasks:
...
You need to generate xref:3.x@apikit-basic-anatomy.adoc#backend-flows[backend flows] for each resource-action pairing in the RAML you create.
