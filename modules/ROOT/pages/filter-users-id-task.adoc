= Filtering Users By ID Using a Parameter
...
+
image::apikit-query-parameter.png["The general option in the request tab is displayed to show the name ID, the value attributes.queryParams.id, and the other defaults."]
. In the properties editor, set metadata properties as follows:
+
* Select Metadata.
+
image::apikit-metadata.png["Metadata option is highlighted in the Request tab."]
+
...
+
image::apikit-set-payload-type.png["Select metadata type window is displayed with the previously mentioned options."]
. Save all.
...
* xref:add-names-service-task.adoc[]
