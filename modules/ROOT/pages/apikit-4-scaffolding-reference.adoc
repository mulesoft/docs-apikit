= APIkit for REST Scaffolding Reference
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:page-aliases: creating-an-apikit-4-project-with-maven.adoc

APIkit for REST scaffolders generate Mule flows and supporting components from RESTful API Modeling Language (RAML) or OpenAPI Specification (OAS) contracts. Use Anypoint Studio to import an API from Anypoint Exchange, Design Center, or local files and produce the implementation backbone automatically. Scaffolding wires the HTTP listener, APIkit Router, API console flow, and per-action flows you replace with business logic.
...
APIkit for REST supports the following MIME types:
