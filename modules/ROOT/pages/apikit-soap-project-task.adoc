...
. Select or accept the default EE Runtime, for example *Mule Server 3.8.1 EE*.
. Check *Add APIkit Components*.
. Browse to the *tshirt2.wsdl* file you downloaded.
. Select a service and port from the drop-down menus. For example, select TshirtService and TshirtServicePort.
+
image::apikit-for-soap-47fb8.png["*New Mule Project* dialog with populated fields."]
+
. Click *Finish*.
+
