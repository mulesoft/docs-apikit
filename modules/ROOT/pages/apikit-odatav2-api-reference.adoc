= Defining EDM with RAML in APIkit for OData v2
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

Define your OData Entity Data Model (EDM) in RAML so APIkit for OData v2 can build the `odata.raml` library the extension uses to scaffold flows. Model entities with EDM-compatible types, keys, and annotations that match the requirements below. After scaffolding with the APIkit OData extension, implement the generated endpoints with your business logic.
...
These annotations pertain to each property:
