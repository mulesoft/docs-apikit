...
This workflow will guide you through the process of creating, implementing and testing a REST API.

For example purposes we are using the free online REST service JSONPlaceholder at `+https://jsonplaceholder.typicode.com+`, which provides mocked user information in JSON format.

== Prerequisites
Make sure to have the following software before going through the steps.
...
+
You can do so either in xref:start-project-task.adoc[API Designer from Design Center], as an xref:api-development-studio.adoc[API Specification Project] in Anypoint Studio or by simply using your favourite text editor, as following.
...
. *Run the APIkit project*. Right-click the project, and select *Run As* > *Mule Application*.
. *Simulate API calls to test it*.
.. In Anypoint Studio, locate the *APIkit Consoles* view and click the `Open console` button under the base point URL `http://localhost:8081/console/`.
+
...
+
.. In *API Summary*, expand `*/userbyid*`. Click `*GET*` for the `*/usersbyid*` resource.
+
.. Depending on the size of the browser window or screen resolution, you will see a `*Try it*` button to expand the view for sending a request. If so, click on it.
.. The *API Console* displays a section where you can add parameters and headers to try a request. Make sure the *Show optional parameters* is checked.
.. Enter a `3` as a query parameter value for the *id* parameter and click `*Send*` button.
+
...
. *Add business logic to the API implementation*. Modify the flows to return actual data from the JSON Placeholder service instead of the examples from the API.
...
The REST API development, implementation and testing is done.
