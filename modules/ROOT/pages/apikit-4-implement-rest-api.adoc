...
//TODO: The links of the following paragraph do not work. Do we have to create these pages?
Design your API specification following the steps in xref:start-project-task.adoc[API Designer from Design Center], or in xref:api-development-studio.adoc[API Specification Project] in Anypoint Studio, or using your favorite text editor:
...
* Check the *Scaffold flows from these API specifications* option if not already selected.
. Click *Finish*.
+
APIkit generates the necessary flows based on the API specification you imported.
. Run your APIkit project by right-clicking the project and selecting *Run As* > *Mule Application*.
...
+
It opens the detail page of the resource showing, among others, the expected JSON example for an HTTP 200 response.
+
// TODO: Outdated image. Replace it.
...
+
// TODO: Break this long sentence into 2 or 3 sentences.
...
* On the *Query Parameters* tab, click on *Switch to expression mode* and paste the following DataWeave script:
+
----
...
+
All user information from the JSON Placeholder service is returned:
+
----
[
   {
...
Your REST API creation, implementation, and testing is complete.
