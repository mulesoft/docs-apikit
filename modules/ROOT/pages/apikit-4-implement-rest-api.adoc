...
//The links of the following paragraph do not work. Do we have to create these pages?
...
* Check the *Scaffold flows from these API specifications* option if not already checked.
. Click *Finish*.
+
APIkit generates the necessary flows based on the API specification you imported.
//Check the following steps. Bolding may be wrong.
...
+
It opens the detail page of the resource showing, among others, the JSON example you expect for an HTTP 200 response.
+
// TODO: Outdated image. Replace it.
...
* Path: Set to `/users`
* Select the Query Parameters tab, click on `Switch to expression mode` and paste the following DataWeave script:
+
----
