...
Unlike the APIkit for OData v2 extension that is based on RAML APIs, APIkit for OData v4 leverages the Entity Data Model (EDM) used by OData services to build HTTP-based data services. The EDM formalizes the resource descriptions and structures within the OData service. However, please note that API Designer and API Console do not support OData APIs for the time being.
