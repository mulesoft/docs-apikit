...
+
You can download an link:{attachmentsdir}/odata.raml[example entity data model] to see exactly how to create the file for this example.
+
...
+
image::creating-an-odata-api-b2e8e.png["api.xml and odata.raml located in Package Explorer."]
+
...
Once deployed, you can also register the OData app in external OData consumer services like https://help.salesforce.com/HTViewHelpDoc?id=platform_connect_add_external_data_source.htm&language=en_US[Salesforce].
...
. Download and unpack the link:{attachmentsdir}/apikit-odata-example-main.zip[compressed Maven project], `apikit-odata-example-main.zip`.
+
The pom.xml and other project files appear in the apikit-odata-example-main directory.
+
...
+
. Browse to and select the POM file from the *apikit-odata-example-main* directory. Click *Finish*.
+
