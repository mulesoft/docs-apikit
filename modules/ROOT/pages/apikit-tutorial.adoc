...
. Click *Try It* label.
+
The baseURI is optional in a RAML-backed API but is required by the the API-Console *Try It* functionality.  *Try It* is disabled if there is no baseURI.
+
image::fv-601.png["Example of the Try It feature being disabled because baseURI is not present."]
+
In the RAML, you can use an arbitrary value, for example +http://www.example.com+ for the baseURI.
+
. Click *GET* to send a test call to get a list of machines.
+
...
. Click image:edit-16x16.gif[Edit button] to edit the *Connector Configuration* global configuration element.
