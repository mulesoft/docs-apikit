...
Build an OData v2 REST API with APIkit for OData and the MySQL walkthrough: generate flows from RAML types, connect to a sample database, and exercise HTTP and OData endpoints. The OData MySQL example packages a working Maven project you import into Anypoint Studio.

== Before You Begin

Install this software to create and use an OData REST API with APIkit:
...
This section walks through implementing an OData-enabled REST API:
...
+
image::list-of-odata-files.png["Three files highlighted in the tree: api.xml in the Mule folder, and api.raml and odatalibrary.raml files in the resource folder."]
