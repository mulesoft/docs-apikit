...
This operation takes the `Date/DateTime` values as strings by default. Select the `java.time object` option to output it as a corresponding `Date` or `DateTime` object in the `java.time` package.
...
By default, a term matches if the field value contains the term, but it doesn’t match if set as `strict` in the configuration. 
