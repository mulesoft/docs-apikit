...
The `Transform to SQL Select` operation turns an inbound OData `GET` into an SQL `SELECT` with bound parameters you pass to the Database Connector. Pair it with the connector’s `select` operation to expose relational data through OData v4 without hand-writing SQL for every query. The message payload is the prepared statement text; attributes carry the parameter map the database driver expects.
...
APIkit supports these OData request elements:
...
| Search transformation enabled | If `true`, this option enables the $search transformation option for a WHERE clause.
| Strict search term match | If `true`, this option enables full term matching for $search. If enabled, terms in the WHERE clause aren't enclosed between '%'.
...
| Date Transform Type a| Values provided, one of:
...
​​The OData `$filter` expression uses comparison, logical, arithmetic, and grouping operators. For a list of operators and their meaning, see https://docs.oasis-open.org/odata/odata/v4.01/cs01/part1-protocol/odata-v4.01-cs01-part1-protocol.html[the OData4 Specification^]. 

APIkit does not support the `has` and `divby` operators.

== Expressions Supported by Transform to SQL Select

For the filtering expressions that OData supports, see the https://docs.oasis-open.org/odata/odata/v4.01/cs01/part1-protocol/odata-v4.01-cs01-part1-protocol.html[specification^]. 
