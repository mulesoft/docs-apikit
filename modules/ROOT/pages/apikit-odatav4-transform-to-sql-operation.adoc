...
| Strict search term match | Enables full term matching for $search. If enabled, terms in the WHERE clause aren't enclosed between '%'.
...
This operation takes the `Date/DateTime` values as strings by default. Select the `java.time object` option to output it as a corresponding Date or DateTime object available under the `java.time` package.
