...
You can transform the input OData `GET` request to a SQL SELECT query with the `Transform to SQL Select` operation. This operation is useful when combined with the `select` operation of the Database Connector. 

image::transform-to-sql-transformer.png["Transform to SQL select element and tab highlighted"]
...
The list of queryable entities, their fields, and the relationships between them are defined in a data model using simple HTTP messages.
...
| Entity set name | Name of the entity set that you fetch #[attributes.entitySetName]
| Page size | A number that defines the amount of results per page during server-side-pagination
| Entity type name | A string obtained from the OData request using #[attributes.odataRequestAttributes], which the entity data model specifies
