...
This operation builds a `where` clause based on the `$search` expression that the system query option provides. The implementation translates the search expression to select all of the elements that have property values that match the condition. 
