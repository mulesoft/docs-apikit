...
APIkit is a toolkit that simplifies the development of API-led applications for Mule developers. It scaffolds applications from a given specification, validates input parameters, and creates xref:munit::munit-scaffold-test-task.adoc[MUnit test suites]. APIkit has a module for each supported web-service type:
...
Although each module works with a family of web services, all modules share common usage patterns. So if you're a Mule developer who works with SOAP APIs, you can work with GraphQL, RAML, OData, or OAS APIs.
