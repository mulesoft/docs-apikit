...
Run sample HTTP POST queries against a packaged APIkit for GraphQL service that keeps an in-memory blog-style dataset for testing. Download and start link:{attachmentsdir}/apikit-graphql-example.jar[APIkit for GraphQL application example] before you run the requests. See https://graphql.org/learn/serving-over-http/[GraphQL over HTTP^] for how POST bodies encode queries and variables.

The walkthrough uses this schema:
