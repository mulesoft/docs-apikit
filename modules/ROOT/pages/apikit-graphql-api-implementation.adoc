...
This query requests the data of all the users, including their address information. In terms of implementation, this is using a Data Loader to fetch the addresses of each user in only one request.
...
This example request creates a new post and asks for some information on it.
...
This use case posts a comment in reply to the post created in the previous step. It also asks for the service to return the body of the comment created and the original post with its list of comments to check that the new one is included.
